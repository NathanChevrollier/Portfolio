<header id="header">
    <div class="container">

      <h1><a href="index.php#header">Nathan Chevrollier</a></h1>
      <!-- Uncomment below if you prefer to use an image logo -->
      <!-- <a href="index.html" class="mr-auto"><img src="assets/img/logo.png" alt="" class="img-fluid"></a> -->
      <h2><span>Etudiant / Alternant </span>en BTS SIO à l'ESPL Angers</h2>

      <nav id="navbar" class="navbar">
        <ul>
          <li><a class="nav-link active" href="index.php#header">Accueil</a></li>
          <li><a class="nav-link" href="index.php#resume">Formation</a></li>
          <li><a class="nav-link" href="index.php#entreprise">Entreprise</a></li>
          <li><a class="nav-link" href="index.php#veille">Veille</a></li>
          <li><a class="nav-link" href="index.php#portfolio">Mes Réalisations</a></li>
          <li><a class="nav-link" href="index.php#competences">Tableau de synthèse</a></li>
          <li><a class="nav-link" href="index.php#contact">Contact</a></li>
        </ul>
        <i class="bi bi-list mobile-nav-toggle"></i>
      </nav><!-- .navbar -->
      <div class="social-links">
        <a target="_blank" href="https://twitter.com/NathanChev69021" class="twitter"><i class="bi bi-twitter"></i></a>
        <a target="_blank" href="https://www.linkedin.com/in/nathan-chevrollier-3ba923277/" class="linkedin"><i class="bi bi-linkedin"></i></a>
        <a target="_blank" href="https://github.com/NathanChevrollier" class="linkedin"><i class='bi bi-github'></i></a>
        <a target="_blank" href="assets/files/CV_Chevrollier_Nathan.pdf" class="linkedin" ><i class="bi bi-download"></i><i class='bi bi-file-earmark-text'></i></a>   
      </div>

  </header>